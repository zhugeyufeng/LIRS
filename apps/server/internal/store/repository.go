...
func (r *Repository) ReviewUser(ctx context.Context, id string, input UserReviewInput) (User, error) {
	...
	if !validRoleGroupName(input.Role, input.GroupName) {
		return User{}, clientError("group leader must have a valid group")
	}
...
func (r *Repository) ExpireStaleReservations(ctx context.Context, maxAge time.Duration) (int, error) {
	...
	total := 0
	for {
		count, err := r.expireStaleReservationsBatch(ctx, maxAge, batchSize)
		total += count
...
func (r *Repository) expireStaleReservationsBatch(ctx context.Context, maxAge time.Duration, batchSize int) (int, error) {
...
func validRoleGroupName(role string, groupName string) bool {
